...
void FeatureMatcher::extractFeatures()
{
  ...
  for( int i = 0; i < images_names_.size(); i++  )
  {
    ...
    //sift_detector->detectAndCompute(img, cv::noArray(), features_[i], descriptors_[i]);

    //extract features color
    feats_colors_[i].reserve(features_[i].size());
    for (unsigned int j = 0; j < features_[i].size(); j++) 
    {
      cv::Point coordinates = features_[i][j].pt;
      feats_colors_[i][j] = img.at<cv::Vec3b>(coordinates.y,coordinates.x);
    }
...
void FeatureMatcher::exhaustiveMatching()
{
  for( int i = 0; i < images_names_.size() - 1; i++ )
  {
    for( int j = i + 1; j < images_names_.size(); j++ )
    {
      ...
      auto descriptor_matcher = cv::DescriptorMatcher::create("BruteForce-Hamming");

      descriptor_matcher->match(descriptors_[i], descriptors_[j], matches);

      //initialize vector of matches in the first and second images
      std::vector<cv::Point2f> pts_first(matches.size());
      std::vector<cv::Point2f> pts_second(matches.size());

      //extract correspondences for the two images
      for (size_t k = 0; k < matches.size(); k++) 
      {
        pts_first[k] = features_[i][matches[k].queryIdx].pt;
        pts_second[k] = features_[j][matches[k].trainIdx].pt;
      }

      cv::Mat K = new_intrinsics_matrix_;

      //vectors of inliners points
      std::vector<uchar> inliers_E, inliers_H;

      //compute homography and essential matrix using RANSAC algorithm
      cv::Mat homography = cv::findHomography(pts_first, pts_second, cv::RANSAC, 1.0, inliers_H);
      cv::Mat essential_matrix = cv::findEssentialMat(pts_first, pts_second, K, cv::RANSAC, 0.999, 3.0, inliers_E);

      //compute number of inlier matches
      for (size_t k = 0; k < matches.size(); k++) 
      {
        if (inliers_E[k] || inliers_H[k])
        {
          inlier_matches.push_back(matches[k]);
        }
      }

      //check if the number of inlier matcher is greater than a threshold
