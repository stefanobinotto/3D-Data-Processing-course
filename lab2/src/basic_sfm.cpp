...
void BasicSfM::solve()
{
  ...
  while( !seed_found )
  {
    ...
    //compute homography and essential matrix and the correspondent inliers
    cv::Mat essential_matrix = cv::findEssentialMat(points0, points1, intrinsics_matrix, cv::RANSAC, 0.999, 0.001, inlier_mask_E);
    cv::Mat homography_matrix = cv::findHomography(points0, points1, cv::RANSAC, 0.001, inlier_mask_H);

    //if the number of inliers of the essential matrix is greater that the homography inliers
    if (cv::countNonZero(inlier_mask_E) > cv::countNonZero(inlier_mask_H))
    {
      //recover rotation matrix and translation vector
      cv::Mat R, t;
      cv::recoverPose(essential_matrix, points0, points1, intrinsics_matrix, R, t);
      
      /*
      const double angle = acos((R.at<double>(0,0) + R.at<double>(1,1) + R.at<double>(2,2) - 1) / 2);

      constexpr double degree_angle = 10;
      constexpr double radiant_angle = degree_angle * (CV_PI / 180);
      */

      //extract the y and z axis translation
      const double t_y = t.at<double>(1);
      const double t_z = t.at<double>(2);

      //check the sideward motion
      if (std::abs(t_y) < std::abs(0.35) && std::abs(t_z) < std::abs(0.8))
      {
        seed_found = true;
        init_r_mat = R;
        init_t_vec = t;
      ...
      if( cam_pose_optim_iter_[cam_idx] > 0 )
      {
        for( auto const& co_iter : cam_observation[cam_idx] )
        {
              ...
              cam_observation[new_cam_pose_idx].find(pt_idx ) != cam_observation[new_cam_pose_idx].end() )
          {
            ...
            //retrieve point of the observation with respect to the new camera
            const int index1 = cam_observation[new_cam_pose_idx][pt_idx] * 2;
            points0[0] = cv::Point2d(observations_[index1], observations_[index1 + 1]);

            //retrieve point of the observation with respect to the camera
            const int index2 = cam_observation[cam_idx][pt_idx] * 2;
            points1[0] = cv::Point2d(observations_[index2], observations_[index2 + 1]);

            //get the axis angle representation for the first camera
            cv::Vec3d axis_angle_repr(cam0_data[0], cam0_data[1], cam0_data[2]);
            //get the translation vector for the first camera
            cv::Vec3d traslation_vector(cam0_data[3], cam0_data[4], cam0_data[5]);
            //initialize the rotation matrix
            cv::Matx<double, 3, 3> rotation_matrix;
            
            //compute the rotation matrix using Rodrigues formula
            cv::Rodrigues(axis_angle_repr, rotation_matrix);

            //save the resulting matrix
            cv::hconcat(rotation_matrix, traslation_vector, proj_mat0);

            //get the axis angle representation for the first camera
            axis_angle_repr.val[0] = cam1_data[0];
            axis_angle_repr.val[1] = cam1_data[1];
            axis_angle_repr.val[2] = cam1_data[2];

            //get the translation vector for the first camera
            traslation_vector.val[0] = cam1_data[3];
            traslation_vector.val[1] = cam1_data[4];
            traslation_vector.val[2] = cam1_data[5];
            
            //compute the rotation matrix using Rodrigues formula
            cv::Rodrigues(axis_angle_repr, rotation_matrix);

            //save the resulting matrix
            cv::hconcat(rotation_matrix, traslation_vector, proj_mat1);

            //triangulate the points using the previously computed matrices
            cv::triangulatePoints(proj_mat0, proj_mat1, points0, points1, hpoints4D);

            //chierality constraint
            const double ratio_z = hpoints4D.at<double>(2,0)/hpoints4D.at<double>(3,0);
            if (ratio_z > 0.0) 
            {
              n_new_pts++;
              pts_optim_iter_[pt_idx] = 1;
  ...
  while( keep_optimize )
  {
    ...
    for( int i_obs = 0; i_obs < num_observations_; i_obs++ )
    {
      ...
      if( cam_pose_optim_iter_[cam_pose_index_[i_obs]] > 0 && pts_optim_iter_[point_index_[i_obs]] > 0 )
      {
        ...
        //initialize cost function for the observed point
        ceres::CostFunction* cost_function = ReprojectionError::Create(
            observations_[i_obs*2],
            observations_[i_obs*2+1]
        );

        //initialize loss (cauchy)
